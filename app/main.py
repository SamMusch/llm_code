...
async def api_chat(
    ...
    Accepts JSON body:
      {
        "message": "...",            # required
        "session_id": "...",         # optional
        "file_ids": "id1,id2",       # optional (comma-separated)
        "tools": "a,b,c",            # optional (comma-separated)
    ...
    # Optional tool gating from UI (comma-separated).
    selected_tools: list[str] = []
    if tools:
        try:
            selected_tools = [t.strip() for t in str(tools).split(",") if t.strip()]
    ...
    # Optional metadata filters from UI (JSON string)
    doc_filters: dict = {}
    if filters:
        try:
            doc_filters = json.loads(str(filters)) or {}
    ...
    # Optional model override from UI Settings
    model_name = (str(model).strip() if model is not None else "") or None
    ...
    # Collect middleware/tool steps for this request.
    steps_buffer: list[dict] = []
    sink_token = attach_step_sink(steps_buffer)
        ...
        async for ev in agen:
            ...
            if ev.get("type") == "step":
                ...
                if isinstance(payload, dict):
                    steps_buffer.append(payload)
                else:
                    steps_buffer.append({"data": payload})
                try:
                    for step in drain_step_sink():
                        if isinstance(step, dict):
                            steps_buffer.append(step)
    ...
    if text_out:
        try:
            history.add_ai_message(text_out, trace_id=trace_id)
            log.info(f"DDB assistant write OK session_id={sid}")
        except Exception as e:
            log.exception(f"DDB assistant write FAILED session_id={sid}: {e}")
